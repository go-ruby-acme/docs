...
`go-ruby-acme/acme` reimplements Ruby's `acme` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-acme/acme` directly, with no Ruby runtime;
