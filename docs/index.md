# go-ruby-acme documentation

**Pure-Go (CGO=0), MRI-faithful reimplementation of Ruby's acme-client gem — the ACME / Let's Encrypt (RFC 8555) protocol client.**

`go-ruby-acme/acme` is a faithful, pure-Go (zero cgo) reimplementation of Ruby's `acme`,
...
!!! success "Status: pure-Go, CGO=0, differential-tested"
    A faithful pure-Go port of Ruby's `acme`, validated against reference Ruby, at 100%
...
```sh
go get github.com/go-ruby-acme/acme
```
...
| Repo | What it is |
| --- | --- |
| [`acme`](https://github.com/go-ruby-acme/acme) | the library — Ruby's `acme` in pure Go |
| [`docs`](https://github.com/go-ruby-acme/docs) | this documentation site (MkDocs Material, versioned with mike) |
| [`go-ruby-acme.github.io`](https://github.com/go-ruby-acme/go-ruby-acme.github.io) | the organization landing page (Hugo) |
| [`brand`](https://github.com/go-ruby-acme/brand) | logo and brand assets |
...
Source lives at [github.com/go-ruby-acme/acme](https://github.com/go-ruby-acme/acme).
